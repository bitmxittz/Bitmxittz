...
#if defined(NDEBUG)
# error "Bitmxittz cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6226e1da4d049f3e132af0a7265c1e05d90035e0afdf8cec3a65a6353c30b1c5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 1); // Bitmxittz: starting difficulty is 1 / 2^19
...
const string strMessageMagic = "Bitmxittz Signed Message:\n";
...
{
    // Bitmxittz: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut to zero after 26 blocks, which will occur in premine
    nSubsidy >>= (nHeight / 26); // Bitmxittz: 26 blocks in all time

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 60; // Bitmxittz: 1 minute
static const int64 nTargetSpacing = 1 * 15; // Bitmxittz: 15 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitmxittz: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitmxittz: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6226e1da4d049f3e132af0a7265c1e05d90035e0afdf8cec3a65a6353c30b1c5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 21 Jun 2017 Flynn Was Told CIA Secrets Despite Worries Over Blackmail";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043834710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b04b6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1498016693;
        ...
        assert(block.hashMerkleRoot == uint256("0xbff496fd17e07598dfd79ddd90054f5f5eb4582ab691de52a5e3273f2196d06a"));
...
unsigned char pchMessageStart[4] = { 0xc2, 0xd1, 0xd3, 0xbd }; // Bitmxittz: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitmxittzMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitmxittzMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitmxittzMiner(CWallet *pwallet)
{
    printf("BitmxittzMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitmxittz-miner");
    ...
    try { loop {
        ...
        printf("Running BitmxittzMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitmxittzMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitmxittzMiner, pwallet));
}
